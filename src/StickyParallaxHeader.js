...
class StickyParallaxHeader extends Component {
    ...
    if (isRenderingTabs && prevPage !== currentPage && isFolded) {
      ...
      setTimeout(() => {
        const scrollTargetPosition =
          rememberTabScrollPosition && this.tabsScrollPosition[currentPage] ? this.tabsScrollPosition[currentPage] : scrollHeight;
  ...
  render() {
    const { backgroundImage, children, header, headerHeight, initialPage, parallaxHeight, tabs, bounces, scrollEvent,
    ...
    return (
      <View style={styles.container}>
        ...
        <AnimatedScrollView
          ...
          onMomentumScrollEnd={() => this.onScrollEndSnapToEdge(scrollHeight)}
          ...
          <ScrollableTabView
            ...
            minScrollHeight={innerScrollHeight}keyboardShouldPersistTaps={keyboardShouldPersistTaps}>
            {!tabs && children}
            {tabs &&
              tabs.map((item) => (
                <View
